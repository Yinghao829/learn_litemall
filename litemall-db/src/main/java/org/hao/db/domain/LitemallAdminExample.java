...
/**
 * AdminExample类（条件构造器）
 * 作用：用于构建动态 SQL 查询条件，是 MyBatis Generator 自动生成的 Criteria 查询辅助类。
 * 核心功能：动态条件组装、支持复杂查询、与Mapper XML配合
 * 使用场景：基础条件查询、动态组合复杂条件、统计数量、删除数据
 * */
public class LitemallAdminExample {
    ...
    /**
     * GeneratedCriteria（基类） → Criteria（具体条件构造器） → Criterion（条件单元）
     * GeneratedCriteria
     * 作用：基类，初始化条件列表
     * 生命周期：每个Example对象生成一次
     * 是否可实例化：有mybatis generator生成，不可直接实例化
     */
    protected abstract static class GeneratedCriteria {
        ...
        protected void addRoleIdsCriterion(String condition, Object value, String property) {
            ...
            roleIdsCriteria.add(new Criterion(condition, value, "org.hao.db.mybatis.JsonStringArrayTypeHandler"));
            allCriteria = null;
        }
        ...
        protected void addRoleIdsCriterion(String condition, Integer[] value1, Integer[] value2, String property) {
            ...
            roleIdsCriteria.add(new Criterion(condition, value1, value2, "org.hao.db.mybatis.JsonStringArrayTypeHandler"));
            allCriteria = null;
        }
     ...
     * Criteria
     * 作用：具体的条件构造器，一组条件
     * 生命周期：每次调用createCriteria()生成一个
     * 是否可实例化：可通过Example实例化
     */
     ...
     * 作用：单个条件单元
     * 生命周期：每个条件对应一个实例
     * 是否可实例化：由addCriteria()内部生成，不可直接操作
     */
    ...
    public interface IExampleWhen {
        void example(org.hao.db.domain.LitemallAdminExample example);
    }
}