...
/**
 * admin类
 * 作用：表示数据库表 admin 的一条记录，是数据库表字段到 Java 对象的映射。
 * 使用场景：插入数据、更新数据、数据传输对象（DTO）
 */
...
public class LitemallAdmin {
    ...
    /**
     * 枚举 Column 是用于定义数据库表字段与Java实体类属性之间的映射关系，
     * 通常用于ORM（对象关系映射）框架（如MyBatis）中。
     * 它统一管理了字段的数据库列名、Java属性名、JDBC类型以及是否需要转义符号（如反引号）。
     * 每个常量对应数据库表中的一列，参数依次为：
     * 数据库列名（如 id）
     * Java实体类属性名（如 id）
     * JDBC类型（如 INTEGER）
     * 是否需要转义列名（如 false 表示不需要反引号包裹）
     */
    public enum Column {
